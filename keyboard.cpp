...
void keyboard::keyboardHandlerRegister(TKeyboardFn function)
{
	g_keyboardFunctions.insert(function);
}

void keyboard::keyboardHandlerUnregister(TKeyboardFn function)
{
...
void OnKeyboardMessage(DWORD key, WORD repeats, BYTE scanCode, BOOL isExtended, BOOL isWithAlt, BOOL wasDownBefore, BOOL isUpNow)
{
	if (key < 255)
	{
		g_keyBoard->m_keys[key].time = GetTickCount64();
		g_keyBoard->m_keys[key].isWithAlt = isWithAlt;
		g_keyBoard->m_keys[key].wasDownBefore = wasDownBefore;
		g_keyBoard->m_keys[key].isUpNow = isUpNow;
	}
}

bool keyboard::KeyDown(DWORD key)
{
	return (key < 255) ? ((GetTickCount64() < m_keys[key].time + MAX_DOWN) && !m_keys[key].isUpNow) : false;
}

bool keyboard::KeyJustUp(DWORD key, bool exclusive)
{
	bool b = (key < 255) ? (GetTickCount64() < m_keys[key].time + NOW_PERIOD && m_keys[key].isUpNow) : false;
...
{
	if (key < 255)
		memset(&m_keys[key], 0, sizeof(m_keys[0]));
}

bool keyboard::KeyPressedOnce(bool& bIsPressed, DWORD vk)
{
	if (KeyPressed(vk))
	{
		if (bIsPressed == false)
