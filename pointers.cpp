...
namespace hbase
{
	pointers::pointers()
	{
		...
		main_batch.add("SC", "48 8B 0D ? ? ? ? 48 8B 01 44 8D 43 01 33 D2 FF 50 40 8B C8", [this](memory::handle ptr)// UPDATED
			{
				m_swapchain = ptr.add(3).rip().as<IDXGISwapChain**>();
			});
		main_batch.add("SH&RSH", "45 33 F6 8B E9 85 C9 B8", [this](memory::handle ptr)// UPDATED
			{
		...
		main_batch.add("NRT&GNH", "48 8D 0D ? ? ? ? 48 8B 14 FA E8 ? ? ? ? 48 85 C0 75 0A", [this](memory::handle ptr)// UPDATED
			{
		...
		main_batch.add("FV", "83 79 18 00 48 8B D1 74 4A FF 4A 18 48 63 4A 18 48 8D 41 04 48 8B 4C CA", [this](memory::handle ptr)// UPDATED
			{
				m_fix_vectors = ptr.as<functions::fix_vectors_t>();
			});
		main_batch.add("PF", "48 8B 05 ? ? ? ? 48 8B 48 08 48 85 C9 74 52 8B 81", [this](memory::handle ptr)// UPDATED
			{
				m_ped_factory = ptr.add(3).rip().as<CPedFactory**>();
			});
		main_batch.add("GTS", "48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC 20 8B FA 85 D2 75 2A 8B 15", [this](memory::handle ptr)// I{DATED
			{
		...
		//	});
		main_batch.add("TK", "48 89 5C 24 ? 57 48 83 EC 20 48 83 B9 ? ? ? ? ? 48 8B D9 74 14", [this](memory::handle ptr)// UPDATED
			{
				m_gta_thread_kill = ptr.as<PVOID>();
			});
		main_batch.add("MSB", "48 8B C8 FF 52 30 84 C0 74 05 48", [this](memory::handle ptr)// UPDATED
			{
				m_model_spawn_bypass = ptr.add(8).as<PVOID>();
			});
		main_batch.add("ISA", "40 38 35 ? ? ? ? 75 0E 4C 8B C3 49 8B D7 49 8B CE", [this](memory::handle ptr)// UPDATED
			{
				m_is_session_started = ptr.add(3).rip().as<bool*>();
			});
		main_batch.add("RBWD", "48 89 74 24 ? 57 48 83 EC 20 48 8B D9 33 C9 41 8B F0 8A", [this](memory::handle ptr)// UPDATED
			{
				m_read_bitbuf_dword = ptr.sub(5).as<functions::read_bitbuf_dword_t>();
			});
		main_batch.add("RBA", "48 89 5C 24 ? 57 48 83 EC 30 41 8B F8 4C", [this](memory::handle ptr)// UPDATED
			{
		...
		//	});
		main_batch.add("REH", "66 41 83 F9 ? 0F 83", [this](memory::handle ptr)// UPDATED
			{
				m_received_event = ptr.as<functions::received_event_t>();
			});
		main_batch.add("SEA", "E8 ? ? ? ? 66 83 7B 08 5B", [this](memory::handle ptr)// UPDATED
			{
		...
		main_batch.add("IDP", "84 C0 74 0D 8D 53 0C", [this](memory::handle ptr)
			{
		...
		main_batch.add("TSE", "48 8B C4 48 89 58 08 44 89 48 20 55 56 57 48 83 EC 30", [this](memory::handle ptr)
			{
